...
class HpSpider(scrapy.Spider):
    name = 'gd_yangjiang_gaoxinqu'
    page = 16

    url = 'http://www.yjgx.gov.cn/zwgk/zdlyxxgk/hjbhxxgk/index_{page}.html'

    basic_info = {'province': 6,
                  'city': 91,
                  'source_webname': '广东阳江高新技术产业开发区政务网',
                  'sp_bm': '阳江市高新区环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.yjgx.gov.cn/zwgk/zdlyxxgk/hjbhxxgk/', callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="xwul"]//li/a')
        for link in links:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="Contentime"])').\
                    ...
                    file_texts = response.xpath(
                        '//div[@id="zoom"]//a[contains(@href, "/") and not(contains(@href, "@"))]')
                    if file_texts:
                        item = right_item(self.basic_info, response, gs_type='受理')
                        msg, attachments = get_files(response, file_texts, 1)
                        item['remark'] = json.dumps({"remark": None, "attachments": {"data": attachments,

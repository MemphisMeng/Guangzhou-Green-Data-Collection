...
网址/url： http://xxgk.yangdong.gov.cn/SortHtml/1/Unit_18.html
备注/note：阳东区人民政府网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_yangjiang_yangdongqu'  
    page = 29 

    url = 'http://xxgk.yangdong.gov.cn/TMP/xxgklist2.shtml?action=list&SS_Path=/-022-010-002&unitsId=18&pp={page}'  

    basic_info = {'province': 6,
                  'city': 91,
                  'source_webname': '阳东区人民政府网',
                  'sp_bm': '阳江市阳东区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="xxgk_nav_con"]/ul/li[@class="mc"]/a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        title = response.xpath('//div[@class="newstitle"]').xpath('string(.)').extract_first(default='')
                        ...
                        for i, data_row in enumerate(data_rows):
                            if i > 0:
                                ...
                                item['date_time'] = response.xpath(
                                    'string(//table[@class="table_suoyin"]//tr[4]/td[@class="pmingcheng"])').\

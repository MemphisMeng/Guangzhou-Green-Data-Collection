...
网址/url： http://www.nanhai.gov.cn/cms/html/11715/column_11715_1.html
备注/note：佛山市南海区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_foshan_nanhaiqu'  
    page = 1

    url = 'http://zht.nanhai.gov.cn/nhhb-three/hpgs/hpgslist?businesstype=2&isstreet=0'

    basic_info = {'province': 6,
                  'city': 80,
                  'source_webname': '佛山市南海区人民政府网',
                  'sp_bm': '佛山市南海区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="div_ContentList"]//li[@class="ContentList_li"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="Content_Title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="div_Content"]')
        files = None
        if title and body:

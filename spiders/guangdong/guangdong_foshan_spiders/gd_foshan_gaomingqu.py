...
网址/url： http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/
备注/note：佛山市高明区政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_foshan_gaomingqu'  

    sl_page = 44
    sl_url = 'http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/slgg/index_{page}.html'

    nsp_page = 43
    nsp_url = 'http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/spqgs/index_{page}.html'

    sp_page = 27
    sp_url = 'http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/sphgs/index_{page}.html'

    basic_info = {'province': 6,
                  'city': 80,
                  'source_webname': '佛山市高明区政府网',
                  'sp_bm': '佛山市高明区环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/slgg/index.html',
                             meta={'gs_type': '受理'}, callback=self.parse_url)
        for page in range(1, self.sl_page):
            yield scrapy.Request(url=self.sl_url.format(page=page), meta={'gs_type': '受理'}, callback=self.parse_url)

        yield scrapy.Request(url='http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/spqgs/index.html',
                             meta={'gs_type': '拟审批'}, callback=self.parse_url)
        for page in range(1, self.nsp_page):
            yield scrapy.Request(url=self.nsp_url.format(page=page), meta={'gs_type': '拟审批'}, callback=self.parse_url)

        yield scrapy.Request(url='http://www.gaoming.gov.cn/gzjg%20/zfgzbm/qhbj/hpgggs/sphgs/index.html',
                             meta={'gs_type': '审批'}, callback=self.parse_url)
        for page in range(1, self.sp_page):
            yield scrapy.Request(url=self.sp_url.format(page=page), meta={'gs_type': '审批'}, callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="lists mt20"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h1[@class="title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="TRS_Editor"]')
        files = None
        if title and body:

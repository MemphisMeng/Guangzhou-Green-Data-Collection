...
网址/url： http://www.ss.gov.cn/zwgk/zdlyxxgkzl/hjbhxxgk/jsxmhjyxpjxx/
备注/note：佛山市三水区政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_foshan_sanshuiqu'  
    page = 29

    url = 'http://www.ss.gov.cn/zwgk/zdlyxxgkzl/hjbhxxgk/jsxmhjyxpjxx/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 80,
                  'source_webname': '佛山市三水区政府网',
                  'sp_bm': '佛山市三水区生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.ss.gov.cn/zwgk/zdlyxxgkzl/hjbhxxgk/jsxmhjyxpjxx/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="articlelist"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h1)').extract_first(default='')

...
网址/url： http://www.maonan.gov.cn/index.php?c=category&id=236
备注/note：茂名市茂南区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_maoming_maonanqu'  
    page = 13

    url = 'http://www.maonan.gov.cn/index.php?c=category&id=236&page={page}'  

    basic_info = {'province': 6,
                  'city': 85,
                  'source_webname': '茂名市茂南区人民政府网',
                  'sp_bm': '茂名市茂南区自然资源局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="newsList_right"]//li[@class="clearfix"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//p[@class="newsContainer_title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="newsContainer_text"]')
        files = None
        if title and body:

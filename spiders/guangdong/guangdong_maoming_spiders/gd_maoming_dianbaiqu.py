...
网址/url： http://www.dianbai.gov.cn/dianbai/zwgk_zdly_hjbh_jsxm/list.shtml
备注/note：茂名市电白区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_maoming_dianbaiqu'  
    page = 14

    url = 'http://www.dianbai.gov.cn/dianbai/zwgk_zdly_hjbh_jsxm/list_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 85,
                  'source_webname': '茂名市电白区人民政府网',
                  'sp_bm': '茂名市电白区环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.dianbai.gov.cn/dianbai/zwgk_zdly_hjbh_jsxm/list.shtml',
                             callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="news_list"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//ucaptitle)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//ucapcontent/div/table')
        files = None
        if title and body:

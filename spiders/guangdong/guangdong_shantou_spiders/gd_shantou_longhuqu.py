...
网址/url： http://www.shantou.gov.cn/epd/jsxmhbgs/list.shtml
备注/note：汕头市生态环境局网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shantou_longhuqu'  
    page = 8 

    url = 'http://www.shantou.gov.cn/epd/jsxmhbgs/list_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 88,
                  'source_webname': '汕头市生态环境局网',
                  'sp_bm': '汕头市生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.shantou.gov.cn/epd/jsxmhbgs/list.shtml', callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="list_right"]//li/a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        ...
        if trs:
            ...
            article = response.xpath('//div[@id="zoomcon"]').xpath('string(.)').extract_first(default='')
            if '附件' in article:
                item = right_item(self.basic_info, response, gs_type='受理')
                item['date_time'] = response.xpath('string(//div[@class="ly_div"]//li[@class="c_time"])'). \

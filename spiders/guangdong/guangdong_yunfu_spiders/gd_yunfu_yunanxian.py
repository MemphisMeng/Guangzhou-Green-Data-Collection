...
网址/url： http://www.gdyunan.gov.cn/index.php/open/lists/id/1754.html
备注/note：郁南县人民政府公众信息网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_yunfu_yunanxian'  
    page = 14 

    url = 'http://www.gdyunan.gov.cn/index.php/open/lists.html?id=1754&p={page}'  

    basic_info = {'province': 6,
                  'city': 92,
                  'source_webname': '郁南县人民政府公众信息网',
                  'sp_bm': '云浮市郁南县环境保护局 '}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="right-content"]/a')
        for link in links:
            title = link.xpath('string(./span[@class="text"])').extract_first(default='')
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@class="news-detail-info-content"]//td[5]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="news-detail-left"])').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@class="news-detail-info-content"]'

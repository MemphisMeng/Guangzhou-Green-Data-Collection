...
网址/url： http://www.luoding.gov.cn/web/ldmh/newslong.ptl?channelCode=009006010006010003
备注/note：罗定市人民政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_yunfu_luodingshi'  
    page = 22 

    url = 'http://www.luoding.gov.cn/web/ldmh/newslong.ptl?channelCode=009006010006010003&pageNo={page}'

    basic_info = {'province': 6,
                  'city': 92,
                  'source_webname': '罗定市人民政府门户网站',
                  'sp_bm': ''}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="tablist"]//li/a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@class="article_txt txt_conna"]//td[5]/../../tr')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//span[@class="title_time"])').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@class="wxts_txt"]//a[contains(@href, "/") '

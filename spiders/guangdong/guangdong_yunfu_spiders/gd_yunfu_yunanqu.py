...
网址/url： http://www.yunan.gov.cn/web/yamh/newslong.ptl?channelCode=009002026009013003001
备注/note：云浮市云安区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_yunfu_yunanqu'  
    page = 14 

    url = 'http://www.yunan.gov.cn/web/yamh/newslong.ptl?channelCode=009002026009013003001&pageNo={page}'  

    basic_info = {'province': 6,
                  'city': 92,
                  'source_webname': '云浮市云安区人民政府网',
                  'sp_bm': '云浮市云安区环境保护局'}
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[contains(@class, "article_txt")]//td[5]/..')
        if trs:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[contains(@class, "article_txt")]//'

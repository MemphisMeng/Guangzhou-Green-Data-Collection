...
网址/url： http://www.qingxin.gov.cn/qingxinquzhongdianlingyxxgk/hjbh/jsxmhjyxpjxx
备注/note：清新区重点领域信息公开专栏
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_qingyuan_qingxinqu'  
    page = 8

    url = 'http://www.qingxin.gov.cn/channel/208005003?pageNo={page}&$$time$$=20190604161441'  

    basic_info = {'province': 6,
                  'city': 87,
                  'source_webname': '清新区重点领域信息公开专栏',
                  'sp_bm': '清远市清新区环境保护局'}
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="c_page"]/h1)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="Section0"]/table//table')
        files = None
        if title and body and '验收' not in title and '次公示' not in title and '公众参与' not in title:

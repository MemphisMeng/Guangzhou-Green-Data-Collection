...
网址/url： http://www.yangshan.gov.cn/yangshanzhengfumenhuwz/xxgk/zdly/hjbh?paramCode=001003014
备注/note：阳山县政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_qingyuan_yangshanxian'  
    page = 24

    url = 'http://www.yangshan.gov.cn/web/mh/muban/channel.ptl?pageNo={page}&' \
          'channelCode=001003014004&$$time$$=20190604150223'

    basic_info = {'province': 6,
                  'city': 87,
                  'source_webname': '阳山县政府门户网站',
                  'sp_bm': '清远市阳山县生态环境局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@class="list3"]/li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="m-title"]/span)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="con"]/table')
        files = None
        if title and body and '监测' not in title and '质量' not in title and '验收' not in title:

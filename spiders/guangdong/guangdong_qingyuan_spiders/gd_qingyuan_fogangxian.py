...
网址/url： http://zwgk.fogang.gov.cn/SortHtml/65/List_247.html
备注/note：佛冈县政府信息公开网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_qingyuan_fogangxian'  
    page = 9

    url = 'http://zwgk.fogang.gov.cn/tmp/Nav_lanmu.shtml?SS_ID=247&s=39729.23&pp={page}'  

    basic_info = {'province': 6,
                  'city': 87,
                  'source_webname': '佛冈县政府信息公开网',
                  'sp_bm': '清远市佛冈县环境保护局'}
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="wztit"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="wzcon"]')
        files = None
        if title and body and '次公示' not in title and '备案管理' not in title:

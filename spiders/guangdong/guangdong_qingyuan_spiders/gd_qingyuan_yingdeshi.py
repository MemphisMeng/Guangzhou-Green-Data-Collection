...
网址/url： http://www.yingde.gov.cn/yingde/jsxmhj/list.shtml
备注/note：英德市人民政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_qingyuan_yingdeshi'  
    page = 24

    url = 'http://www.yingde.gov.cn/yingde/jsxmhj/list_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 87,
                  'source_webname': '英德市人民政府门户网站',
                  'sp_bm': '清远市英德市环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="list-right"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="title_cen"]/ucaptitle)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="zoomcon"]/table')
        files = None
        if title and body and '次公示' not in title:

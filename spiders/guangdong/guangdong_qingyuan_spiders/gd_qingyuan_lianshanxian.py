...
网址/url： http://www.gdls.gov.cn/content/column/4697450?pageIndex=1
备注/note：连山壮族瑶族自治县人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_qingyuan_lianshanxian'  
    page = 3

    url = 'http://www.gdls.gov.cn/content/column/4697450?pageIndex={page}'  

    basic_info = {'province': 6,
                  'city': 87,
                  'source_webname': '连山壮族瑶族自治县人民政府网',
                  'sp_bm': '清远市连山壮族瑶族自治县自然资源局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@id="mm2"]//li/a')
        for link in links:
            date_time = link.xpath('string(./../span[@class="right date"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        ...
        files = None
        if title and body and '验收' not in title and '情况公示' not in title and '环罚字' not in title \
                and '许可证' not in title and '环责停' not in title and '随机抽查' not in title:

...
网址/url： http://www.luhe.gov.cn/lhzdly/hjbhxx/lhlist2.shtml
备注/note：陆河县人民政府网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shanwei_luhexian'  
    page = 18 

    url = 'http://www.luhe.gov.cn/lhzdly/hjbhxx/lhlist2_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 89,
                  'source_webname': '陆河县人民政府网站',
                  'sp_bm': '汕尾市陆河县环境保护局'}
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@id="zoomon"]//td[5]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="ly_div"]//li[@class="c_time"])').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@id="zoomon"]//a[contains(@href, "/") '
            ...
            if response.xpath('//div[@id="zoomon"]//a[contains(@href, "/") and not(contains(@href, "@"))]'):
                item = right_item(self.basic_info, response, gs_type='受理')
                item['date_time'] = response.xpath('string(//div[@class="ly_div"]//li[@class="c_time"])'). \
                    re_first('\d{4}\D\d{1,2}\D\d{1,2}')  # 发布时间
                file_texts = response.xpath('//div[@id="zoomon"]//a[contains(@href, "/") '

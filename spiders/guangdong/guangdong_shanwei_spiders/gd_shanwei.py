...
网址/url： http://www.shanwei.gov.cn/hbj/
备注/note：汕尾市生态环境局网站
"""
...
class HpSpider(scrapy.Spider):
    ...
    url = 'http://www.shanwei.gov.cn/hbj/System/NewsList.aspx?classid=440&page={page}'  

    basic_info = {'province': 6,
                  'city': 89,
                  'source_webname': '汕尾市生态环境局网站',
                  'sp_bm': '汕尾市生态环境局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="newsclass"]//li//a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@class="content"]//td[5]/..')
        if trs:

...
网址/url： http://www.kaiping.gov.cn/kphbj/gsxx_28310/jsxmspgs_28311/index.html
备注/note：开平市政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_kaipingshi'  
    page = 25

    url = 'http://www.kaiping.gov.cn/kphbj/gsxx_28310/jsxmspgs_28311/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '开平市政府网',
                  'sp_bm': '江门市开平市生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.kaiping.gov.cn/kphbj/gsxx_28310/jsxmspgs_28311/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="text_intercept"]/a')
        for link in links:
            date_time = link.xpath('string(./../../div)').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@id="printArea"]/div[1]/span)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="tpl-content"]')
        files = None
        if title and body and '公众参与' not in title and '委托' not in title and '跟踪' not in title:

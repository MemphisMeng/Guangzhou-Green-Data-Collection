...
网址/url： http://www.enping.gov.cn/zwgk/zdlygk/hjbhxxgk/jsxmhjyxpjxx/index.html
备注/note：恩平市政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_enpingshi'  
    page = 25

    url = 'http://www.enping.gov.cn/zwgk/zdlygk/hjbhxxgk/jsxmhjyxpjxx/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '恩平市政府门户网站',
                  'sp_bm': '江门市恩平市环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.enping.gov.cn/zwgk/zdlygk/hjbhxxgk/jsxmhjyxpjxx/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="csq_morelist"]//li/a')
        for link in links:
            date_time = link.xpath('string(./../spam)').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="csq_content"]/h1)').extract_first(default='')

...
网址/url： http://113.107.142.41/jhq/bwbj/gthbj/xxgk_147558/hjbhxx/index.html
备注/note：江海区政务信息网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_jianghaiqu'  
    page = 25

    url = 'http://113.107.142.41/jhq/bwbj/gthbj/xxgk_147558/hjbhxx/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '江海区政务信息网',
                  'sp_bm': '江门市江海区国土资源和环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://113.107.142.41/jhq/bwbj/gthbj/xxgk_147558/hjbhxx/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//ul[@class="pagelist1"]/li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="article_title"]/h2)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="article_con"]')
        files = None

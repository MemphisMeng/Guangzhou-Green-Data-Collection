...
网址/url： http://www.heshan.gov.cn/zwgk/zdlyxxgk/hjbh/jsxmhjyxpjxx/index.html
备注/note：鹤山市人民政府门户网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_heshanshi'  
    page = 15

    url = 'http://www.heshan.gov.cn/zwgk/zdlyxxgk/hjbh/jsxmhjyxpjxx/index_14.html'  

    basic_info = {'province': 6,
                  'city': 84,
                  'source_webname': '鹤山市人民政府门户网',
                  'sp_bm': '江门市鹤山市环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.heshan.gov.cn/zwgk/zdlyxxgk/hjbh/jsxmhjyxpjxx/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="col-content"]/a[@class="link"]')
        for link in links:
            date_time = link.xpath('string(./span[@class="time"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="ue_table"]')
        files = None
        if title and body:

...
网址:  受理  http://hbj.jiangmen.gov.cn/thirdData/zdlhxxgk/jsxmhjyxpjxx/jsxmhpspslgg/
备注:  江门市生态环境局网站
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen'  # 地名
    page = 30   # 由于页数随时可能出现变动，最好做全局变量以方便记忆

    url = 'http://hbj.jiangmen.gov.cn/thirdData/zdlhxxgk/jsxmhjyxpjxx/jsxmhpspslgg/index_{page}'  # 爬虫起始地址，翻页网站

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '江门市生态环境局网站',
                  'sp_bm': '江门市生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://hbj.jiangmen.gov.cn/thirdData/zdlhxxgk/jsxmhjyxpjxx/jsxmhpspslgg/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="right_content"]//li/a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@class="content_text"]/table//td[7]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string()').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@class="content_text"]//a[contains(@href, "/")'

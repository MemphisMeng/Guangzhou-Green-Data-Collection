...
网址/url： http://www.pjq.gov.cn/ztzl/zdlyxxgk/hjbhxxgk/jsxmhjyx/index.html
备注/note：江门市蓬江区政务网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_pengjiangqu'  
    page = 12

    url = 'http://www.pjq.gov.cn/ztzl/zdlyxxgk/hjbhxxgk/jsxmhjyx/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '江门市蓬江区政务网',
                  'sp_bm': '江门市蓬江区生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.pjq.gov.cn/ztzl/zdlyxxgk/hjbhxxgk/jsxmhjyx/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="ty_wbk_newslist"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="sub_news_title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="zoomCont"]')
        files = None
        if title and body:

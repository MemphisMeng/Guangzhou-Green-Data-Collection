...
网址/url： http://www.xinhui.gov.cn/zwgk/zdlyxxgk/hjbh_1/jsxm/slgg/index.html
备注/note：新会区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_xinhuiqu'  
    page = 14

    url = 'http://www.xinhui.gov.cn/zwgk/zdlyxxgk/hjbh_1/jsxm/slgg/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '新会区人民政府网',
                  'sp_bm': '江门市新会区环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.xinhui.gov.cn/zwgk/zdlyxxgk/hjbh_1/jsxm/slgg/index.html',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="gl-con"]//span[@class="li-left"]/a')
        for link in links:
            date_time = link.xpath('string(./../../span[@class="li-right"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="article-title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="art-con"]')
        files = None
        if title and body:

...
网址/url： http://www.cnts.gov.cn/cnts/jsxmhjyxpjxx/list2.shtml
备注/note：台山政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jiangmen_taishanshi'  
    page = 74

    url = 'http://www.cnts.gov.cn/cnts/jsxmhjyxpjxx/list2_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 83,
                  'source_webname': '台山政府网',
                  'sp_bm': '江门市台山市环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.cnts.gov.cn/cnts/jsxmhjyxpjxx/list2.shtml', callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="list_right"]//div[@class="list_right_title"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        ...
        body = response.xpath('//div[@id="zoom"]/ucapcontent/div')
        files = None
        if title and body:

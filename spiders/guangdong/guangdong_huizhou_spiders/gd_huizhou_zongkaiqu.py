...
网址/url： http://zwgk.hzzk.gov.cn/zkzdly/jsxmhp/hb_list.shtml
备注/note：仲恺高新区管委会门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_huizhou_zongkaiqu'  
    page = 21

    url = 'http://zwgk.hzzk.gov.cn/zkzdly/jsxmhp/hb_list_{page}.shtml'

    basic_info = {'province': 6,
                  'city': 82,
                  'source_webname': '仲恺高新区管委会门户网站',
                  'sp_bm': '惠州市仲恺高新区生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://zwgk.hzzk.gov.cn/zkzdly/jsxmhp/hb_list.shtml', callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@id="fathernodid8"]//a')
        for link in links:

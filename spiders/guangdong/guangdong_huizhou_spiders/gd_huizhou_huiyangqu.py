...
网址/url： http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpsl/index.shtml
备注/note：惠州市惠阳区人民政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_huizhou_huiyangqu'

    sl_page = 55
    sl_url = 'http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpsl/index_{page}.shtml'

    nsp_page = 41
    nsp_url = 'http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpsqgs/index_{page}.shtml'

    sp_page = 1
    sp_url = 'http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpshgg/index_{page}.shtml'

    basic_info = {'province': 6,
                  'city': 82,
                  'source_webname': '惠州市惠阳区人民政府门户网站',
                  'sp_bm': '惠州市惠阳区生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpsl/index.shtml',
                             callback=self.parse_url)
        for page in range(2, self.sl_page + 1):
            yield scrapy.Request(url=self.sl_url.format(page=page), callback=self.parse_url)

        yield scrapy.Request(url='http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpsqgs/index.shtml',
                             callback=self.parse_url)
        for page in range(2, self.nsp_page + 1):
            yield scrapy.Request(url=self.nsp_url.format(page=page), callback=self.parse_url)

        yield scrapy.Request(url='http://www.huiyang.gov.cn/gzjg/qzf/ghbj/hpgs/hpshgg/index.shtml',
                             callback=self.parse_url)
        # for page in range(2, self.sp_page + 1):
        #     yield scrapy.Request(url=self.sp_url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@id="class-list"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@id="news-title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="news-content"]')
        files = None
        if title and body:

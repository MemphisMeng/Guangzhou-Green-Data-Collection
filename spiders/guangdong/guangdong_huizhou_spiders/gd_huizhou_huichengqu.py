...
网址/url： http://zwgk.hcq.gov.cn/hczdly/jsxmhp/hb_list.shtml
备注/note：惠州市惠城区政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_huizhou_huichengqu'  
    page = 56

    url = 'http://zwgk.hcq.gov.cn/hczdly/jsxmhp/hb_list_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 82,
                  'source_webname': '惠州市惠城区政府网',
                  'sp_bm': '惠州市惠城区生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://zwgk.hcq.gov.cn/hczdly/jsxmhp/hb_list.shtml', callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@id="fathernodid8"]')
        for link in links:

...
网址/url： http://zwgk.dayawan.gov.cn/6004/dywList.shtml
备注/note：惠州市大亚湾区环境保护信息公开目录系统
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_huizhou_dayawanqu'  
    page = 56

    url = 'http://zwgk.dayawan.gov.cn/dywzdly/jsxmhp/hb_list_{page}.shtml'  

    basic_info = {'province': 6,
                  'city': 82,
                  'source_webname': '惠州市大亚湾区环境保护信息公开目录系统',
                  'sp_bm': '惠州市大亚湾区环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://zwgk.dayawan.gov.cn/dywzdly/jsxmhp/hb_list.shtml', callback=self.parse_url)
        for page in range(2, self.page + 1):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@id="fathernodid8"]//td/a')
        for link in links:
    ...
    def parse_type(self, response):
        ...
        body = response.xpath('//div[@class="zwgk_comr3 line"]')
        files = None
        if title and body:

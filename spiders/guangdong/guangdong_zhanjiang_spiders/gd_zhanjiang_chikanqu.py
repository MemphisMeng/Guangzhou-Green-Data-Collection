...
class HpSpider(scrapy.Spider):
    name = 'gd_zhanjiang_chikanqu'  # 地名
    page = 1   # 由于页数随时可能出现变动，最好做全局变量以方便记忆

    url = 'http://www.chikan.gov.cn/jsxmhjyx.shtml'  # 爬虫起始地址，翻页网站

    basic_info = {'province': 6,
                  'city': 93,
                  'source_webname': 'http://www.chikan.gov.cn/jsxmhjyx.shtml',
                  'sp_bm': '湛江市赤坎区环保局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="mt9 mv8"]/ul/li/a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//table[@class="\&quot;MsoNormalTable\&quot;"]//td[7]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="mv6"])').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@id="info"]//a')
                                num = len(data_rows) - 1

...
class HpSpider(scrapy.Spider):
    name = 'gd_zhanjiang_potouqu'  # 地名
    page = 1   # 由于页数随时可能出现变动，最好做全局变量以方便记忆

    url = 'http://www.ptq.gov.cn/Modules/items/keyAreaList.aspx?cid=e7b61f84-f6b3-45fb-b653-5ab697715232'  # 爬虫起始地址，翻页网站

    basic_info = {'province': 6,
                  'city': 93,
                  'source_webname': '坡头区人民政府网',
                  'sp_bm': '湛江市坡头区环保局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@id="ul_datalist"]//a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@id="newsContentCon]/table//td[7]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="dqwz clearfix"]/../p)').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@id="newsContentCon]//a[contains(@href, "/")'

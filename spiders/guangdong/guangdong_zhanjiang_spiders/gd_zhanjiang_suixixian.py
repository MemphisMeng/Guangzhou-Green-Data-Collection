...
class HpSpider(scrapy.Spider):
    name = 'gd_zhanjiang_suixixian'  # 地名
    page = 10   # 由于页数随时可能出现变动，最好做全局变量以方便记忆

    url = 'http://www.suixi.gov.cn/Category_159/Index_{page}.aspx'  # 爬虫起始地址，翻页网站

    basic_info = {'province': 6,
                  'city': 93,
                  'source_webname': '遂溪县政府网',
                  'sp_bm': '湛江市遂溪县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="bd"]//li')
        for link in links:
            title = link.xpath('string(./a[2])').extract_first(default='').strip()
            url = link.xpath('./a/@href').extract_first(default='')
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//div[@class="content"]//td[7]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="info"])').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@class="content"]//a[contains(@href, "/")'

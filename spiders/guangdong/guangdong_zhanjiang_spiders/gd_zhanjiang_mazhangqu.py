...
网址:  受理  http://www.zjmazhang.gov.cn
备注:  麻章区人民政府
...
class HpSpider(scrapy.Spider):
    name = 'gd_zhanjiang_mazhangqu'  # 地名
    page = 1   # 由于页数随时可能出现变动，最好做全局变量以方便记忆

    url = 'http://www.zjmazhang.gov.cn/MZ2016/keyProject.aspx?cid=4b13a097-6d1e-440e-a68d-ede1055babb4&' \
          'fcid=51e6c6ce-6f70-4d60-bcb3-ffd85981af1e'  # 爬虫起始地址，翻页网站

    basic_info = {'province': 6,
                  'city': 93,
                  'source_webname': '麻章区人民政府',
                  'sp_bm': '湛江市麻章区环保局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="column_cor"]//li')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//table[@class="MsoTableGrid"]//td[7]/..')
        if trs:
                ...
                if is_test:
                    ...
                    for i, data_row in enumerate(data_rows):
                        if i > 0:
                            ...
                            item['date_time'] = response.xpath('string(//div[@class="entry-meta"])').\
                            ...
                            if file_texts:
                                ...
                                file_texts = response.xpath('//div[@id="tinyview_content_id"]//a[contains(@href, "/")'

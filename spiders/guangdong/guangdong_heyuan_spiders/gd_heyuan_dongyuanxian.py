...
网址/url： http://www.gddongyuan.gov.cn/index.php?s=news&c=category&id=102&page=1
备注/note：东源县政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_heyuan_dongyuanxian'  
    page = 10

    url = 'http://www.gddongyuan.gov.cn/index.php?s=news&c=category&id=102&page={page}'  

    basic_info = {'province': 6,
                  'city': 78,
                  'source_webname': '东源县政府网',
                  'sp_bm': '河源市东源县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="zy_list"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="zy_nr_tit"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="zoomcon"]//table')
        files = None
        if title and body:

...
网址/url： http://www.gdmx.gov.cn/Home/ChannelContent?cid=332
备注/note：梅州市梅县区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_meizhou_meixianqu'
    page = 11

    url = 'http://www.gdmx.gov.cn/Home/ChannelContent?cid=332&page={page}'  

    basic_info = {'province': 6,
                  'city': 86,
                  'source_webname': '梅州市梅县区人民政府网',
                  'sp_bm': '梅州市梅县区环境保护局'}
    ...
    def parse_url(self, response):
        ...
        for link in links:
            date_time = link.xpath('string(./../p[@class="date "])').re_first('d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="list-contentbox"]/h1[1])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="news_content"]/table//td[6]/ancestor::table[1]')
        files = None

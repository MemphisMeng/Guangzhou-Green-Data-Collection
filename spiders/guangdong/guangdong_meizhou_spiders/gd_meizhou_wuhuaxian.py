...
网址/url： http://www.wuhua.gov.cn/zwgk/zd/hjbh/index.htm
备注/note：五华县人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_meizhou_wuhuaxian'  
    page = 29

    url = 'http://www.wuhua.gov.cn/zwgk/zd/hjbh/index_{page}.htm'  

    basic_info = {'province': 6,
                  'city': 86,
                  'source_webname': '五华县人民政府网',
                  'sp_bm': '梅州市五华县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="tong pt10 xxk_txt pr20 listcon_right"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="text_main"]/h3)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="Zoom"]/table')
        files = None
        if title and body and '备案系统' not in title and '监测' not in title \
                and '次公示' not in title and '排名' not in title:

...
网址/url： http://www.pingyuan.gov.cn/zwgk/hjkjsxm/index.html
备注/note：平远县人民政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_meizhou_pingyuanxian'  
    page = 12

    url = 'http://www.pingyuan.gov.cn/zwgk/hjkjsxm/{page}.html'  

    basic_info = {'province': 6,
                  'city': 86,
                  'source_webname': '平远县人民政府门户网站',
                  'sp_bm': '梅州市平远县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@class="list_new data_list"]/a')
        for link in links:
            date_time = link.xpath('string(./li/p[@class="list_new_data"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="header"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="artical"]/table')
        files = None
        if title and body:

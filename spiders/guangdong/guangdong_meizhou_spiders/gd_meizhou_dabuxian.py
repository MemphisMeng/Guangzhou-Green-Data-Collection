...
网址/url： http://www.dabu.gov.cn/index.php?m=content&c=index&a=lists&catid=503
备注/note：大埔县人民政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_meizhou_dabuxian'  
    page = 5

    url = 'http://www.dabu.gov.cn/index.php?m=content&c=index&a=lists&catid=503&page={page}'  

    basic_info = {'province': 6,
                  'city': 86,
                  'source_webname': '大埔县人民政府门户网站',
                  'sp_bm': '梅州市大埔县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@class="jrdb_one"]/li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="nr_main"]/h1)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="bottom"]/table')
        files = None
        if title and body and '次公示' not in title and '公报' not in title and '条例' not in title \
                and '指南' not in title and '报告书' not in title:

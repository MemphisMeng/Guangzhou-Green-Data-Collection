...
网址/url： http://www.meijiang.gov.cn/Home/ChannelContent?cid=580
备注/note：梅州市梅江区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_meizhou_meijiangqu'

    sl_page = 39
    sl_url = 'http://www.meijiang.gov.cn/Home/ChannelContent?cid=580&page={page}'

    nsp_page = 40
    nsp_url = 'http://www.meijiang.gov.cn/Home/ChannelContent?cid=581&page={page}'

    sp_page = 20
    sp_url = 'http://www.meijiang.gov.cn/Home/ChannelContent?cid=582&page={page}'

    basic_info = {'province': 6,
                  'city': 86,
                  'source_webname': '梅州市梅江区人民政府网',
                  'sp_bm': '梅州市梅江区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@class="list"]/li/a')
        for link in links:
            date_time = link.xpath('string(./../p[@class="date "])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@id="content_toplitile"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="news_content"]')
        files = None
        if title and body:

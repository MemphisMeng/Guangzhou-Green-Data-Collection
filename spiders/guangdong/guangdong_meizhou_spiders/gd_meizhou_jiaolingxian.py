...
# date: 2019/4/2
# 
# 网址:  受理  http://www.jiaoling.gov.cn/html/huanjbh/index.html
# 备注:  蕉岭县人民政府门户网站
# 
# """
# 
...
#     name = 'gd_meizhou_jiaolingxian'  
#     page = 33 
# 
#     url = 'http://www.jiaoling.gov.cn/html/huanjbh/index_{page}.html'  
# 
#     basic_info = {'province': 6,
#                   'city': 86,
#                   'source_webname': '蕉岭县人民政府门户网站',
#                   'sp_bm': '梅州市蕉岭县环保局'}
# 
...
#         yield scrapy.Request(url='http://www.jiaoling.gov.cn/html/huanjbh/index.html', callback=self.parse_url)
#         for page in range(2, self.page + 1):
#             yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)
# 
#     def parse_url(self, response):
#         links = response.xpath('//div[@class="xyh_listul"]//li/a')
#         for link in links:
...
#         trs = response.xpath('//div[@id="Zoom"]//td[5]/..')
#         if trs:
...
#                             item['date_time'] = response.xpath('string(//div[@class="pages-date"])').\
...
网址/url： http://www.jiaoling.gov.cn/html/huanjbh/index.html
备注/note：蕉岭县人民政府门户网站
"""
...
class HpSpider(scrapy.Spider):
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@class="currency_ul"]/li/a')
        for link in links:
            date_time = link.xpath('string(./../span/em[@class="abtFlag"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
            url = link.xpath('./@href').extract_first(default='')
            full_url = response.urljoin(url)
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h2/em[@class="abtFlag"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="Zoom"]/table')
        files = None
        if title and body and '验收' not in title and '登记表' not in title and '处罚' not in title and '决定' \
                not in title and '案' not in title and '环境信息' not in title:

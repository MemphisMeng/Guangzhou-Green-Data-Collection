...
网址/url： https://www.fengshun.gov.cn/zwgk/zdlyxxgk/hjbhxx/jsxmhjyxpjxx/index.htm
备注/note：丰顺县人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_meizhou_fengshunxian'  
    page = 3

    url = 'https://www.fengshun.gov.cn/zwgk/zdlyxxgk/hjbhxx/jsxmhjyxpjxx/index_{page}.htm'  

    basic_info = {'province': 6,
                  'city': 86,
                  'source_webname': '丰顺县人民政府网',
                  'sp_bm': '梅州市丰顺县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@id="replace_content"]//li/a[@class="link"]')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h2[@class="mb20 tc fz24"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="news_content"]/table')
        files = None
        if title and body and '次公示' not in title and '验收' not in title \
                and '意见' not in title and '次公布' not in title:

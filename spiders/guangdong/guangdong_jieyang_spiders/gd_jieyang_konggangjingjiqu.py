...
# date: 2019/3/29
# 
# 网址:  受理  http://zwgk.jykgjjq.gov.cn/openInfo.action?depCode=090132268
# 备注:  空港经济区政府信息公开目录系统
# 
# """
# 
...
#     name = 'gd_jieyang_konggangjingjiqu'  # Python文件名
#     page = 106 
# 
#     url = 'http://zwgk.jykgjjq.gov.cn/OpenInfoRight.action?pageNum={page}&pageSize=10&depCode=090132268&isLawDoc=&publisher=&menuCat=&indexNum=&startDate=&endDate=&=title='  
# 
#     basic_info = {'province': 6,
#                   'city': 84,
#                   'source_webname': '空港经济区政府信息公开目录系统',
#                   'sp_bm': '揭阳空港经济区环境保护和安全生产监管局'}
# 
...
#         links = response.xpath('//table[@class="csstable"]//tr//a')
#         for link in links:
...
#         trs = response.xpath('//div[@class="cont"]//td[5]/..')
#         if trs:
...
#                             item['date_time'] = response.xpath('string(//table[@class="datatable"]/tr[2])').\
...
#                             else:
#                                 file_texts = response.xpath('//div[@class="add"]/a')
#                                 num = len(data_rows) - 1
...
网址/url： http://zwgk.jykgjjq.gov.cn/openInfo.action?depCode=090132268
备注/note：揭阳空港经济区信息公开目录系统
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jieyang_konggangjingjiqu'  
    page = 65

    url = 'http://zwgk.jykgjjq.gov.cn/OpenInfoRight.action?pageNum={page}&pageSize=10&depCode=090132268&menuCat=611'  

    basic_info = {'province': 6,
                  'city': 84,
                  'source_webname': '揭阳空港经济区信息公开目录系统',
                  'sp_bm': '揭阳空港经济区环境保护和安全生产监管局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//table[@class="csstable"]//td[@class="tdl"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="tit"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="cont"]/table')
        files = None
        if title and body:

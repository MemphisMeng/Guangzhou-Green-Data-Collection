...
网址/url： http://www.jyrongcheng.gov.cn/List.aspx?q_pi=1&q_searchtype=&q_atttype=&q_cid=781
备注/note：揭阳市榕城区人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jieyang_rongchengqu'  
    page = 21

    url = 'http://www.jyrongcheng.gov.cn/List.aspx?q_pi={page}&q_searchtype=&q_atttype=&q_cid=781'  

    basic_info = {'province': 6,
                  'city': 84,
                  'source_webname': '揭阳市榕城区人民政府网',
                  'sp_bm': '揭阳市榕城区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="zwli2_main"]//span[@class="lleft"]/a')
        for link in links:
            date_time = link.xpath('string(./../../span[@class="lright"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="view_content"]/h2)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="view_content"]/span/table')
        files = None
        if title and body:

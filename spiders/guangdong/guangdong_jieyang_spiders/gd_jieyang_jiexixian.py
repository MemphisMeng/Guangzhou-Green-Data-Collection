...
日期/date：2019/3/28
修改者/modifier：
修改日期/update date：2019/5/31

网址/url： http://zdly.jiexi.gov.cn/zdly/openInfo.action?depFlag=hjbh&menuCat=53&catalogue=hjbhgk
备注/note：揭阳市揭西县重点领域信息公开专栏
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_jieyang_jiexixian'  
    page = 25

    url = 'http://zdly.jiexi.gov.cn/zdly/OpenInfoRight.action?indexNum=&startDate=&endDate=&title=&' \
          'publisher=&menuCat=53&depCode=007029615&isLawDoc=&depFlag=hjbh&pager.offset={page}'

    basic_info = {'province': 6,
                  'city': 84,
                  'source_webname': '揭阳市揭西县重点领域信息公开专栏',
                  'sp_bm': '揭阳市揭西县环境保护局'}
    ...
    def start_requests(self):
        for page in range(self.page):
            yield scrapy.Request(url=self.url.format(page=page * 10), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//table[@class="csstable"]//tr/td[@class="tdl"]/a')
        for link in links:

...
网址/url： https://zdly.sg.gov.cn/website/zdly/govPublicZdly!titletype_more.action?typecode=56&css=hjbh&siteOrgCode=440281
备注/note：乐昌市人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shaoguan_lechangshi'  
    page = 87 

    url = 'https://zdly.sg.gov.cn/website/zdly/govPublicZdly!titletype_more.action?siteOrgCode=440281'  

    basic_info = {'province': 6,
                  'city': 90,
                  'source_webname': '乐昌市人民政府网',
                  'sp_bm': '韶关市乐昌市环境保护局'}
    ...
    def parse_url_sl(self, response):
        ...
        if '受理' in title and ('环境影响' in title or '环评' in title):
            trs = response.xpath('//div[@class="cyy_contnet_trct"]//td[5]/../../tr')
            if trs:

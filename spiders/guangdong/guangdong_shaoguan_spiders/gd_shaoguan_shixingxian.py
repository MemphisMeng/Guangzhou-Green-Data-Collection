...
网址/url： https://zdly.sg.gov.cn/website/zdly/govPublicZdly!titletype_more.action?typecode=56&css=hjbh&siteOrgCode=440222
备注/note：始兴县人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shaoguan_shixingxian'  
    page = 35 

    url = 'https://zdly.sg.gov.cn/website/zdly/govPublicZdly!titletype_more.action?' \
          'typecode=56&css=hjbh&siteOrgCode=440222'

    basic_info = {'province': 6,
                  'city': 90,
                  'source_webname': '始兴县人民政府网',
                  'sp_bm': '韶关市始兴县环境保护局'}
    ...
    def start_requests(self):
        for page in range(1, self.page + 1):
            ...
            yield scrapy.FormRequest(url=self.url.format(page=page), formdata=formdata, callback=self.parse_url)
    ...
    def parse_url_sl(self, response):
        ...
        if '受理' in title and ('环境影响' in title or '环评' in title):
            trs = response.xpath('//div[@class="Section0"]//td[5]/../../tr')
            if trs:

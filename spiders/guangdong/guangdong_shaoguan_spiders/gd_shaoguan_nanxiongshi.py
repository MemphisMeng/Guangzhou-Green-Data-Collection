...
网址/url： https://zdly.sg.gov.cn/website/zdly/govPublicZdly!titletype_more.action?typecode=56&css=hjbh&siteOrgCode=440282
备注/note：南雄市人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shaoguan_nanxiongshi'  
    page = 220

    url = 'https://zdly.sg.gov.cn/website/zdly/' \
          'govPublicZdly!titletype_more.action?typecode=56&css=hjbh&siteOrgCode=440282'

    basic_info = {'province': 6,
                  'city': 90,
                  'source_webname': '南雄市人民政府网',
                  'sp_bm': '韶关市南雄市生态环境局'}
    ...
    def parse_url_sl(self, response):
        ...
        if '受理' in title and ('环境影响' in title or '环评' in title):
            trs = response.xpath('//div[@class="cyy_contnet_trct"]//td[4]/../../tr')
            if trs:
                ...
                if is_same_td:
                    ...
                    is_test = ('项目' in test2 or '规划' in test2) and '建设地点' in test3 \
                              and '建设单位' in test4 and ('环评' in test5 or '环境影响' in test5)
                    if is_test:
                        ...
                        for i, data_row in enumerate(data_rows):
                            if i > 0:
                                ...
                                if file_texts:
                                    ...
                                    file_texts = response.xpath('//div[@class="cyy_contnet_trct"]//'
                        ...
                        if file_texts:
                            ...
                            yield problem_item(self.basic_info, response, gs_type='受理',
                ...
                file_text = response.xpath('//div[@class="cyy_contnet_trct"]//a[contains(@href, "/")'
                                           ' and not(contains(@href, "@"))]')
                if file_text:
                    item = right_item(self.basic_info, response, gs_type='受理')
                    msg, attachments = get_files(response, file_text, 1)
                    item['remark'] = json.dumps({"remark": None, "attachments": {"data": attachments,
                                                 "msg": msg}}, ensure_ascii=False)
                    yield item
                else:

...
网址/url： http://www.yantian.gov.cn/icatalog/bm/nlswj/hjbh/jsxmhjyxpj/
备注/note：盐田区政务公开网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shenzhen_yantianqu'  
    page = 63

    url = 'http://www.yantian.gov.cn/icatalog/bm/nlswj/hjbh/jsxmhjyxpj/index_{page}.htm'

    basic_info = {'province': 6,
                  'city': 77,
                  'source_webname': '盐田区政务公开网',
                  'sp_bm': '深圳市盐田区水务局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.yantian.gov.cn/icatalog/bm/nlswj/hjbh/jsxmhjyxpj/index.htm',
                             callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="row pl15 pr15"]//div[@class="news-item-temp1 whiteBg p20"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h2[@class="tit"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="articleCentent article-enter-table"]')
        files = None
        if title and body:

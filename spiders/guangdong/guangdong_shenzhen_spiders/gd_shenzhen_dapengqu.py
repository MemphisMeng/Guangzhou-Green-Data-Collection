...
网址/url： http://www.dpxq.gov.cn/xxgk/zdly/hjbh/yxpj/index.htm
备注/note：大鹏新区政府在线
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shenzhen_dapengqu'  
    page = 50

    url = 'http://www.dpxq.gov.cn/xxgk/zdly/hjbh/yxpj/index_{page}.htm'  

    basic_info = {'province': 6,
                  'city': 77,
                  'source_webname': '大鹏新区政府在线',
                  'sp_bm': '深圳市大鹏新区生态环境局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url="http://www.dpxq.gov.cn/xxgk/zdly/hjbh/yxpj/index.htm", callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="zx_ml_list"]//li/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="tit"]/h1)').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="news_cont_d_wrap"]')
        files = response.xpath('//div[@class="fujian_downcon"]//a[contains(@href, "/") or contains(@href, "?")]')
        if title and body:

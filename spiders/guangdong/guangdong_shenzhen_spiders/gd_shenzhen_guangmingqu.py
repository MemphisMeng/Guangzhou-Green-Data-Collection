...
网址/url： http://apps.szgm.gov.cn/szgm/132100/152326/152474/152498/index.html
备注/note：光明区政府在线网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_shenzhen_guangmingqu'  
    page = 419

    url = 'http://apps.szgm.gov.cn/szgm/132100/152326/152474/152498/ad93b30c-{page}.html'  

    basic_info = {'province': 6,
                  'city': 77,
                  'source_webname': '光明区政府在线网',
                  'sp_bm': '深圳市光明区环境保护和水务局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@id="modelone"]//li[@class="easysite-article-li"]/p[@class="article-title"]/a')
        for link in links:
            date_time = link.xpath('string(./../span[@class="article-time"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="titxixi"])').extract_first(default='')

...
网址/url： http://www.raoping.gov.cn/Category_1001/Index.aspx
备注/note：饶平县人民政府网站
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_chaozhou_raopingxian'  
    page = 7

    url = 'http://www.raoping.gov.cn/Category_1001/Index_{page}.aspx'  

    basic_info = {'province': 6,
                  'city': 78,
                  'source_webname': '饶平县人民政府网站',
                  'sp_bm': '潮州市饶平县环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//table[@class="pubTable"]//tr//a')
        for link in links:
            date_time = link.xpath('string(./../../td[@class="datetime"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h3[@class="title"])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@id="fontzoom"]')
        files = None
        if title and body:

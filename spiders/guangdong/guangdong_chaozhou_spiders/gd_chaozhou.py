...
网址:  受理  http://www.chaozhou.gov.cn/jsxmhjyxpjxx/index.jhtml
备注:  中国潮州政府网站
...
class HpSpider(scrapy.Spider):
    name = 'gd_chaozhou_shouli'  # 地名
    page = 21   # 由于页数随时可能出现变动，最好做全局变量以方便记忆

    url = 'http://www.chaozhou.gov.cn/jsxmhjyxpjxx/index_{page}.jhtml'  # 爬虫起始地址，翻页网站
    ...
    def parse_url(self, response):
        links = response.xpath('//span[contains(@class,"guifanxing-list")]/../a')
        for link in links:
    ...
    def parse_url_sl(self, response):
        trs = response.xpath('//span[@id="zoom"]/table//td[7]/..')
        if trs:

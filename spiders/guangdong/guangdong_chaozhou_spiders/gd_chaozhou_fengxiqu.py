...
日期/date：2019/3/20
修改者/modifier：
修改日期/update date： 2019/5/30

网址/url： http://www.fengxi.gov.cn/zdly/zdlylist.aspx?pagenum=1&itemid=538&deptid=119&cateid=0&searchtype=0
备注/note：枫溪区管理委员会公众网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_chaozhou_fengxiqu'  
    page = 2

    url = 'http://www.fengxi.gov.cn/zdly/zdlylist.aspx?pagenum={page}&itemid=538&deptid=119&cateid=0&searchtype=0'  

    basic_info = {'province': 6,
                  'city': 78,
                  'source_webname': '枫溪区管理委员会公众网',
                  'sp_bm': '潮州市枫溪区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="date"]/../..//a')
        for link in links:
            date_time = link.xpath('string(./../..//div[@class="date"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="limit-box"]/../table[1])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="limit-box"]/../table//table//table')
        files = None
        if title and body:

...
网址/url： http://www.chaoan.gov.cn/zdly/more.aspx?itemid=219
备注/note：潮州市潮安区人民政府公众网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_chaozhou_chaoanqu'  
    page = 1

    url = 'http://www.chaoan.gov.cn/zdly/zdlylist.aspx?pagenum={page}&itemid=319&deptid=119&cateid=0&searchtype=0'  

    basic_info = {'province': 6,
                  'city': 78,
                  'source_webname': '潮州市潮安区人民政府公众网',
                  'sp_bm': '潮州市潮安区环境保护局'}
    ...
    def parse_type(self, response):
        title = response.xpath('string(//div[@class="limit-box"]//tr[4]/td[2])').extract_first(default='')
        date_time = response.meta['date_time']
        body = response.xpath('//div[@class="limit-box"]/../table[3]')
        files = None
        if title and body:

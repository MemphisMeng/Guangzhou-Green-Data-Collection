...
网址/url： http://www.sihui.gov.cn/zwgk_22209/zdlyxxgk_22272/hjbh_22278/jsxmhjyxpjxx_22681/index.html
备注/note：肇庆市四会市人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'gd_zhaoqing_sihuishi'
    page = 50 

    url = 'http://www.sihui.gov.cn/zwgk_22209/zdlyxxgk_22272/hjbh_22278/jsxmhjyxpjxx_22681/index_{page}.html'  

    basic_info = {'province': 6,
                  'city': 94,
                  'source_webname': '肇庆市四会市人民政府网',
                  'sp_bm': '肇庆市四会市环境保护局'}
    ...
    def start_requests(self):
        yield scrapy.Request(url='http://www.sihui.gov.cn/zwgk_22209/zdlyxxgk_22272/hjbh_22278/'
                                 'jsxmhjyxpjxx_22681/index.html', callback=self.parse_url)
        for page in range(1, self.page):
            yield scrapy.Request(url=self.url.format(page=page), callback=self.parse_url)

    def parse_url(self, response):
        links = response.xpath('//div[@class="content ty_text"]//li/a')
        for link in links:
            title = link.xpath('string(.)').extract_first(default='')
            url = link.xpath('./@href').extract_first(default='')
            full_url = response.urljoin(url)
            if ('受理' in title or '审批前' in title) and ('环境影响' in title or '环评' in title):
                yield scrapy.Request(url=full_url, callback=self.parse_url_sl)
            elif '公示' not in title:
                yield scrapy.Request(url=full_url, callback=self.parse_url_sl)

...
网址/url： http://www.masbwq.gov.cn/Home/List/11?cid=14
备注/note：博望区政务公开网
"""
...
class HpSpider(scrapy.Spider):
    name = 'ah_maanshan_bowangqu'
    page = 9

    url = 'http://bwqzwgk.mas.gov.cn/opennessTarget/?branch_id=57a3df762c262ea9a00aadfc&column_code=350503&page={page}'

    basic_info = {'province': 3,
                  'city': 46,
                  'source_webname': '博望区政务公开网',
                  'sp_bm': '马鞍山市博望区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//ul[@class="z-list"]/li/span[@class="title"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        title = response.xpath('string(//h2)').extract_first(default='')

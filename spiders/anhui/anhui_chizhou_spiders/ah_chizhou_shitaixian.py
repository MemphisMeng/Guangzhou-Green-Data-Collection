...
网址/url： http://www.ahshitai.gov.cn/TMP/nav_gongkailanmu.shtml?unitsId=1&SSIDS=2045&action=list&SS_Path=/-012-151-014-004
备注/note：石台县人民政府网
"""
...
class HpSpider(scrapy.Spider):
    name = 'ah_chizhou_shitaixian'  
    page = 10 

    url = 'http://www.ahshitai.gov.cn/tmp/xxgklist2.shtml?unitsId=1&action=list&SS_Path=/-012-151-014-004&types=2&pp={page}'  

    basic_info = {'province': 3,
                  'city': 39,
                  'source_webname': '石台县人民政府网',
                  'sp_bm': '池州市石台县生态环境局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//div[@class="xxgk_nav_con"]//li[@class="mc"]/a')
        for link in links:
    ...
    def parse_type(self, response):
        ...
        body = response.xpath('//div[@id="Zoom2"]')
        files = None
        if title and body:

...
网址/url： http://zwgk.tl.gov.cn:81/web/unit.aspx?unit=003101085&sort=2&xxflid=070203&subjectid=&page=1
备注/note：铜陵市政府信息公开网
"""
...
class HpSpider(scrapy.Spider):
    name = 'ah_tongling_tongguanqu'  
    page = 9

    url = 'http://zwgk.tl.gov.cn:81/web/xxgk/unit.aspx?unit=003101085&sort=2&xxflid=070203&subjectid=&page={page}'  

    basic_info = {'province': 3,
                  'city': 48,
                  'source_webname': '铜陵市政府信息公开网',
                  'sp_bm': '铜陵市铜官区环境保护局'}
    ...
    def parse_url(self, response):
        links = response.xpath('//form[@id="aspnetForm"]//td[@class="pad"]//a')
        for link in links:
            date_time = link.xpath('string(./../../../td[@class="pad"])').re_first('\d{4}\D\d{1,2}\D\d{1,2}')
            url = link.xpath('./@href').extract_first(default='')
            full_url = 'http://zwgk.tl.gov.cn:81/web/' + url

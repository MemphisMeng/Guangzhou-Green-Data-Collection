...
def format_table(table):
    ...
    '''
    # 初始化一个空列表，二维列表，对应table中的 tr/td
    # 替换table中除了tr/td外的其他元素
    data_list = []
    new_table = table.xpath('.').extract_first(default='')
    new_table = new_table.replace('<thead', '').replace('</thead', '').replace('<th', '<td').replace('</th', '</td'). \
        replace('<tbody>', '').replace('</tbody>', '').replace('<tfoot>', '').replace('</tfoot>', '')

    # 遍历所有table中的td, 添加到二维列表中
    trs = Selector(text=new_table).xpath('//body//tr/td[2]/..')
    ...
    # 获取data_list中每个tr中td的span属性情况，并将span属性大于1的情况储存到data_list_span
    data_list_span = []
    for i, tr in enumerate(data_list):
        data_list_span.append([])
        for j, td in enumerate(tr):
            td_span = Selector(text=td).xpath('.//@rowspan').extract_first() or Selector(text=td).xpath('.//@colspan').extract_first()
            if td_span:
                data_list_span[i].append(td_span)
    # 如果tr中有3个以上有span属性的td，返回原来的表格
...
def split_span(table):
    ...
    for i, tr in enumerate(data_list):
        data_list_span.append([])
        for j, td in enumerate(tr):
            td_span = Selector(text=td).xpath('.//@rowspan').extract_first() or Selector(text=td).xpath(
                './/@colspan').extract_first()
            if td_span:
                data_list_span[i].append(td_span)
    # 如果tr中有3个以上有span属性的td，返回原来的表格
    for i in data_list_span:
